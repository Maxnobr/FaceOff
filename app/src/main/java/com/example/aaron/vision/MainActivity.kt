...
class MainActivity : AppCompatActivity() {
    ...
    inner class GVisionThread:AsyncTask<String, String, String>() {
        ...
        private var facts:List<FaceAnnotation>? = null
        ...
        override fun doInBackground(vararg arg: String?):String {
            ...
            desiredFeature.type = "FACE_DETECTION"
            ...
            Log.i(TAG, "Number returned is ${batchResponse!!.responses.size}, response[0] is ${batchResponse.responses[0]}")
            returnedInfo = "${batchResponse.responses[0]}"     //returned JSon info
            facts = batchResponse.responses[0].faceAnnotations       //list of picture labels
            return returnedInfo
        }
     ...
     */
    private fun buildDescription(labelList: List<FaceAnnotation>?) {
        ...
        if (gDescription.isNotEmpty()) {        // != null) {
            ...
            gDescription.forEach{
                ...
                if (msg.isNotEmpty()) {   //add new line between descriptors
                    msg += "\n"
                }
                msg += it       //"$desc\n"
            }
    ...
    private fun readJsonStream(inStream: InputStream) {
    ...
    private fun readDescriptionArray(reader: JsonReader) {
        gDescription = ArrayList()
     ...
     */
    @Throws(IOException::class)
    private fun readMessage(reader: JsonReader): String {
        val map = linkedMapOf<String,String>()
        val blank = "N/A"
        map["joyLikelihood"] = blank
        map["sorrowLikelihood"] = blank
        map["angerLikelihood"] = blank
        map["surpriseLikelihood"] = blank
        map["underExposedLikelihood"] = blank
        map["blurredLikelihood"] = blank
        map["headwearLikelihood"] = blank
    ...
    companion object {
        ...
        private const val API_KEY = "AIzaSyDAAQzPxXFxJ9Dx65de6BaaEdn4zFnWEis"//your API Key should be placed here
    }
